...
if(isset($_GET['delete_id'])) {
    $penulis_id = $_GET['delete_id'];
    
    // Check if author has books
    $check_stmt = $connection->prepare("SELECT COUNT(*) as book_count FROM buku WHERE penulis_id = ?");
    ...
    if($result['book_count'] > 0) {
        $_SESSION['error'] = "Tidak dapat menghapus penulis karena masih memiliki buku terkait!";
    } else {
        $stmt = $connection->prepare("DELETE FROM penulis WHERE penulis_id = ?");
        $stmt->bind_param("i", $penulis_id);
        if($stmt->execute()) {
            $_SESSION['message'] = "Penulis berhasil dihapus!";
        } else {
...
// Get all authors
$stmt = $connection->prepare("SELECT p.*, COUNT(b.buku_id) as total_buku FROM penulis p LEFT JOIN buku b ON p.penulis_id = b.penulis_id GROUP BY p.penulis_id ORDER BY p.name");
$stmt->execute();
...
<head>
    ...
    <title>Manajemen Penulis - Sistem Manajemen Perpustakaan</title>
                        ...
                        <div class="card-body">
                            <div class="row align-items-center">
                                <div class="col-md-8">
                                    ...
                                    <p class="text-muted mb-0">Kelola data penulis buku dalam sistem perpustakaan digital</p>
                                </div>
                            ...
                            <div class="row align-items-center">
                                ...
                                <div class="col-md-6">
                                    <div class="search-box">
                                        ...
                                        <input type="text" class="form-control" placeholder="Cari penulis..." id="searchInput">
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <table class="table table-hover mb-0" id="authorsTable">
                                ...
                                <tbody>
                                    <?php if(empty($authors)): ?>
                                        <tr>
                                            <td colspan="6" class="text-center py-4">
                                                <i class="bi bi-pencil display-4 text-muted d-block mb-2"></i>
                                                <span class="text-muted">Belum ada penulis terdaftar</span>
                                                <div class="mt-3">
                                                    <a href="add_author.php" class="btn btn-primary btn-sm">
                                                        <i class="bi bi-plus-circle me-2"></i>
                                    ...
                                    <?php else: ?>
                                        ...
                                        <tr>
                                            <td>
                                                <span class="badge bg-light text-dark">#<?php echo $author['penulis_id']; ?></span>
                                            </td>
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    ...
                                                    <div class="flex-grow-1 ms-3">
                                                        <h6 class="mb-0"><?php echo htmlspecialchars($author['name']); ?></h6>
                                                        <?php if(!empty($author['bio'])): ?>
                                                            <small class="text-muted"><?php echo htmlspecialchars(substr($author['bio'], 0, 50)); ?>...</small>
                                                        <?php endif; ?>
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                                <?php if(!empty($author['email'])): ?>
                                                    <a href="mailto:<?php echo htmlspecialchars($author['email']); ?>" class="text-decoration-none">
                                            ...
                                            <td>
                                                <span class="book-count">
                                                    ...
                                                    <?php echo $author['total_buku']; ?> buku
                                                </span>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                <div class="d-flex gap-2 justify-content-center">
                                                    <a href="edit_penulis.php?id=<?php echo $author['penulis_id']; ?>" class="btn btn-outline-primary btn-action" title="Edit Penulis">
                                                        <i class="bi bi-pencil"></i>
                                                    </a>
                                                    <a href="manage_authors.php?delete_id=<?php echo $author['penulis_id']; ?>" 
                                                       class="btn btn-outline-danger btn-action <?php echo $author['total_buku'] > 0 ? 'disabled' : ''; ?>" 
                                                       title="<?php echo $author['total_buku'] > 0 ? 'Tidak dapat menghapus penulis yang memiliki buku' : 'Hapus Penulis'; ?>"
                                                       onclick="<?php echo $author['total_buku'] > 0 ? 'return false;' : "return confirm('Apakah Anda yakin ingin menghapus penulis " . htmlspecialchars($author['name']) . "?')"; ?>">
                ...
                <div class="col-md-3">
                    <div class="card form-card animate-fade-in-up" style="animation-delay: 0.2s;">
                        <div class="card-body text-center">
                            ...
                            <h4 class="mb-1"><?php echo count($authors); ?></h4>
                ...
                <div class="col-md-3">
                    <div class="card form-card animate-fade-in-up" style="animation-delay: 0.4s;">
                        <div class="card-body text-center">
                            ...
                            ?>
                            <h4 class="mb-1"><?php echo $authors_with_books; ?></h4>
                            <p class="text-muted mb-0">Penulis Aktif</p>
                        </div>
                    </div>
                </div>
                <div class="col-md-3">
